...
public class GeneratorMenu extends AbstractContainerMenu {
    ...
    protected GeneratorMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(GeneratorBlockEntity.DATA_CONTAINER_COUNT)); //psize has to match the count in blockentity
    }
    ...
    public GeneratorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuRegistry.GENERATOR_MENU.get(), pContainerId); //the first one needs to be the matching menu type to the block entity type
        checkContainerSize(inv, GeneratorBlockEntity.SLOT_COUNT);//pMinSize is how many slots your block entity has, i wonder if you can use a public final to not have to type the same number twice
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, GeneratorBlockEntity.FUEL_SLOT, 46, 35)); //fuel slot
            this.addSlot(new SlotItemHandler(handler, GeneratorBlockEntity.CHARGING_SLOT, 118, 35)); //charger slot
        });
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = GeneratorBlockEntity.SLOT_COUNT;  // the number of slots in our block entity
    ...
    public boolean isCrafting() {
        return data.get(GeneratorBlockEntity.burnTimeCase) > 0;
    }
    //returns the progress of the craft in how many pixels of the progress bar should be displayed
    public int getScaledBurnHeight(int height){
        int burnTime = data.get(GeneratorBlockEntity.burnTimeCase);
        int maxBurnTime = data.get(GeneratorBlockEntity.maxBurnTimeCase);

        //maxProgress != 0 && progress !=0 ? progress*(progressBarHeight/maxProgress) : 0;
        return height * burnTime/maxBurnTime;
    }
    public int getScaledEnergyHeight(int height){
        int energyStored = data.get(GeneratorBlockEntity.energyCase);
        int maxEnergy = GeneratorBlockEntity.MAX_ENERGY_STORE;
        return maxEnergy != 0 && energyStored != 0 ? height * energyStored/maxEnergy : 0;
    }
